...
def linear_attention(qs, ks, vs):
    ...
    attention_num = torch.einsum("nm,md->nd", qs, kvs) 
    all_ones = torch.ones([vs.shape[0]]).to(vs.device)
    vs_sum = torch.einsum("l,ld->d", all_ones, vs) 
    attention_num += vs_sum.unsqueeze(0).repeat(vs.shape[0], 1) 
    all_ones = torch.ones([ks.shape[0]]).to(ks.device)
    ks_sum = torch.einsum("lm,l->m", ks, all_ones)
    attention_normalizer = torch.einsum("nm,m->n", qs, ks_sum) 
...
class MLP_encoder(nn.Module):
    def __init__(self, nfeat, nhid, dropout):
        ...
        self.Wk = nn.Linear(nfeat, nhid)
        self.Wq = nn.Linear(nfeat, nhid)
        self.Wv = nn.Linear(nfeat, nhid)
    def forward(self, o_x, x):
        query = self.Wq(o_x)
        key = self.Wk(o_x)
        value = self.Wv(o_x)
        attention_output = linear_attention(query, key, value)
        x = self.Linear1(x)
